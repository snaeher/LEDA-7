...
#define LEDA_ROOT_INCL_ID 721061
...
array(const std::initializer_list<E>& lst) { 
   construct(0,lst.size()-1);
   int i = 0;
   for(E x: lst) set(i++,x); 
}
/*{\Mcreate creates  an instance |\Mvar| of type |\Mname| and initializes it to 
            a copy of $lst$, e.g. $array<int> A({1,2,3,4,5})$ }*/
...
void sort(std::function<bool(const E&,const E&)> smaller, int low, int high) {
  quicksort(vec+low-l,vec+high-l,smaller); 
}
...
void sort(bool (*smaller)(const E&,const E&),int low, int high) { 
  quicksort(vec+low-l,vec+high-l,smaller);
}
