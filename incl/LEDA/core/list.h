...
#define LEDA_ROOT_INCL_ID 721060
...
int  elem_size()           const { return sizeof(list_elem); }
bool elem_destructible()   const { return std::is_destructible<E>::value; }
...
{ return ((list_elem*)p)->inf < ((list_elem*)q)->inf; }
};
...
list(const std::initializer_list<E>& lst) { for(E x: lst) append(x); }
/*{\Mcreate creates  an instance |\Mvar| of type |\Mname| and initializes it to 
            a copy of $lst$, e.g. $list<int> L({1,2,3,4,5})$ }*/
...
// move constructor
list(list<E>&& a) : dlist(std::move(a)) {}
...
template<class SMALLER, class SORTALG> 
void list_sort1(const SMALLER& smaller, const SORTALG& sort_alg)
{
  int n = length();
  if (n <= 1) return;
  ...
  item* p = A;
  item it;
  forall_items(it,*this) *p++ = it;

/*
  auto it_smaller = [this,smaller](item p, item q) { return smaller(inf(p),inf(q)); };
*/

  item_smaller it_smaller;
  sort_alg(A,p-1,it_smaller);
...
{ smaller_cmp_obj<E,leda_cmp_base<E> > smaller(cmp);
  list_sort(smaller,QuickSort());
}

void  sort(bool (*smaller)(const E&,const E&)) {
  list_sort(smaller,QuickSort()); 
}

void  sort(std::function<bool(const E&,const E&)> smaller) {
  list_sort(smaller,QuickSort());
}

/*
void  sort(int (*cmp)(const E&,const E&))
{ smaller_cmp_func<E> smaller(cmp);
  list_sort(smaller,QuickSort());
}
*/

void  sort(int (*cmp)(const E&,const E&))
{ auto smaller = [cmp](const E& x, const E& y) { return cmp(x,y) < 0; };
  list_sort(smaller,QuickSort());
}
...
/*{\Mop      sorts the items of $L$ using the default ordering of type $E$,
             ...
             If $E$ is a user-defined type, you have to provide a compare
...
/*{\Mop     sorts the items of $L$ using merge sort and the ordering
            ...
            $\Litem{x}$ is before $\Litem{y}$ in $L$ then $\Litem{x}$ is 
            before $\Litem{y}$ after the sort. |L.merge_sort()| is more 
            efficient than |L.sort()| if $L$ contains large pre-sorted 
            intervals. }*/
...
/*{\Mopl     sorts the items of $L$ using bucket sort, where $b$ maps every 
             element $x$ of $L$ to a bucket $b(x) \in [i..j]$. If $b(x) < b(y)$
             then $\Litem{x}$ appears before $\Litem{y}$ after the sort.
	     If $b(x) = b(y)$, the relative order of $x$ and $y$ before the 
             sort is retained, thus the sort is stable.}*/

void  bucket_sort(int (*b)(const E&)) 
{ ord_ptr = b; dlist::bucket_sort(); }
/*{\Mopl     sorts |\Mname| into increasing order as prescribed by |b| 
             \precond |b| is an integer-valued function on $E$. }*/
...
void  merge(list<E>& L1) { smaller_default<E> smaller; merge(L1,smaller); }
