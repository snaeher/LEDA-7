...
typedef void (*BIN_TREE_UPDATE)(string s);
...
// set operations
  ...
  bin_tree() {  update_func = 0; count = 0; set_root(nil); set_min(nil); }
  ...
  void set_update_func(BIN_TREE_UPDATE f) { update_func = f; }
...
{ bin_tree_node* r = q->child[1-dir];
  ...
  if (update_func) update_func(string("SINGLE ROTATE: dir = %d",dir));
 }
...
{ int dir2 = 1-dir1;
  ...
  if(x != &ROOT ) propagate_modification(11,x,r);

  if (update_func) update_func(string("DOUBLE ROTATE: dir = %d",dir1));
}
