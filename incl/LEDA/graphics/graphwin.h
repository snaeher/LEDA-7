...
friend class __exportC gml_gwgraph;
...
void edit_loop(int event, int but, double x, double y);
void handle_button_event(int but, double x, double y);
...
int open_panel(panel& P, bool close);

int open_panel(panel& P) { return open_panel(P,true); }
...
int add_menu(string label, int menu_id=0, char* pmap=0, const char* hlp=0);
/*{\Mop	... }*/
...
void  load_gw_graph(string fname);
void  load_gml_graph(string fname);
void  load_dimacs_graph(string fname);

void  save_graph(string fname);
void  save_gw_graph(string fname);
void  save_gml_graph(string fname);
void  save_dimacs_graph(string fname);
...
int read_gw(string fname) { 
  return read(fname,true); 
}
...
{ bool old = get_directed();
  ...
  set_edge_direction((b) ? directed_edge : undirected_edge,false);
  return old;
}
...
list_item get_found_bend()  const { return found_bend; }
...
list_item find_bend(const point& p, edge x);
...
friend __exportF void gw_upload_handler(GraphWin& gw);
...
friend __exportF void gw_save_gw_handler(GraphWin& gw);
...
__exportF void gw_upload_handler(GraphWin& gw);
...
__exportF void gw_save_gw_handler(GraphWin& gw);
