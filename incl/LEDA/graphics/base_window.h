...
typedef void   (*win_close_handler_func)(BASE_WINDOW*);
...
class __exportC tooltip {
  ...
  double y1;
  string text;
  tooltip(int i, const char* txt, double x0, double y0, double x1, double y1)
  { id = i;
    text = txt;
    left = x0;
    bottom = y0;
...
win_close_handler_func win_close_handler;
window_handler          *win_close_ptr;
...
// real window dimensions when buffering
int window_width_save;
int window_height_save;
...
static float screen_dpi_scaling() { return screen_dpi()/192.0f; }

static string display_type();

static void upload(string text);
static void download(string text);
static void keyboard(int x);

static char* root_pixrect(int,int,int,int);



int display_fd() { return disp_fd; }
...
void enable_close_button(bool b) { close_button_enabled = true; }
...
win_close_handler_func set_window_close_handler(win_close_handler_func f);
const window_handler*    set_window_close_object(const window_handler& obj);
...
int get_panel_bg_color()  const { return panel_bg_color;  }
...
int  get_buf_level() { return buf_level; }

void flush_pixels(int,int,int,int);
...
int   menu_style;       // menu style (-1: scrollbar, 0,1,2: menu)
int   menu_size;
...
void set_menu_style(int s)   { menu_style = s; }
...
void set_min_panel_width(int w) { min_panel_width = w; }
...
void enable_button(int b);
void disable_button(int b);
...
int         get_default_width(panel_item);
...
panel_item  get_active_item() { return active_item; }
