                                         ...
                                         edge_array<double>& y_tanch)
{
    ...
    if (!PLANAR(G))
    { error_handler(0,"visibility_rep: Graph is not planer");
      return false;
     }

    if (!Is_Simple(G))
    { error_handler(0,"visibility_rep: Graph is not simple");
      return false;
     }
...
int main()
{
  ...
  while(gw.edit())
  {
    ...
    VISIBILITY_REPRESENTATION(G,x_pos,y_pos,x_rad,y_rad,x_sanch,y_sanch,
                                                        x_tanch,y_tanch);
