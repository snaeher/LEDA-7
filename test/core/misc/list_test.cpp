...
list<int> create_and_move() {
   list<int> L;
   for(int i=0; i<100; i++) L.append(i);
   return L;
}
...
int main () 
{
  for(int x: {1,2,3,4,5}) cout << x << " ";
  cout << endl;

  for(int x: list<int>({1,2,3,4,5})) cout << x << " ";
  cout << endl;
...
/*
  L = create_and_move();
  L.print(cout); cout << endl;
*/

  // range-based loop
  for(double const& x: L2) { cout << x << endl; }
...
/*
  int A[] = { 1,2,3,4,5 };
  //auto smaller = [A](const int& i,const int& j) { return A[i] < A[j]; };
*/

  cout << "sorting(lambda)    ";
  cout.flush();
  auto smaller = [](int i, int j) { return i < j; };
  L.sort(smaller);
  cout << string("  %5.3f sec",used_time(T));
  cout << endl;
