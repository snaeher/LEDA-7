...
{ 
  ...
  if (!indep)
    LEDA_EXCEPTION(1,"FIVE_COLORING: internal error (no independent neighbors).");
}
...
/*
  if (!PLANAR((graph&)G)) {
    LEDA_EXCEPTION(1,"FIVE_COLORING: graph must be planar.");
  }
*/
...
//
    ...
    if (small_deg.empty())
     LEDA_EXCEPTION(1,"FIVE_COLORING: no node with degree < 5 (graph not planar).");
    ...
    if (deg[v] == 5) {
      ...
      if (w == u) { 
	// parallel edges ?
        LEDA_EXCEPTION(1,"FIVE_COLORING: internal error (merging identical nodes)");
      }
      ...
      forall_adj_nodes(x,w) {
	if (x == u) 
         LEDA_EXCEPTION(1,string("FIVE_COLORING: internal error (merging adjacent nodes %d and %d  v = %d",G.index(u), G.index(w),G.index(v)));
        if (mark[x]) { 
	  deg[x]--;
   ...
   while ( ! removed.empty() ) {
     ...
     if (c == 5) 
       LEDA_EXCEPTION(1,"FIVE_COLORING: internal error (more than 5 colors).");
...
static alg_five_color FIVE_COLORING;
...
int main() 
{
  ...
  gw.display_help_text("gw_five_coloring");

  string msg = "Please construct or load a graph ";
  msg += "(e.g. File --> Open --> five.gw).\\n ";
  msg += "Then press 'done' to step through the algorithm.";
  gw.message(msg);
...
*/
  ...
  gw_fc_observer observer(FIVE_COLORING,gw);
  node_array<int> col(gw.get_graph());
      
  FIVE_COLORING(gw.get_graph(),col);

  //gw.wait();

  gw.message("\\bf Coloring completed.");
