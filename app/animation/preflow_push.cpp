...
string doc_file = "Docs/preflow_push_anim.doc";
...
*/
    win.draw_ctext(x,y2, string("%d", (int)cur_height[v]),
                                      cur_col[v].text_color());
                    ...
                    const edge_array <double> &cur_flow, node u) 
{
  ...
  for (i=0; i<steps; i++) {
    forall_nodes (v, G) {
      cur_height[v] = cur_height[v] + ((double)dist[v]-cur_height[v])/(steps-i);
    }
                 ...
                 node u1, node u2, edge mark) 
{
  ...
  for (int i=0; i<steps; i++) {   
    node v;
    forall_nodes (v, G) {
      cur_exc[v] = cur_exc[v] + ((double)excess[v]-cur_exc[v])/(steps-i);
    }
     edge e;
    forall_edges (e, G) {
      cur_flow[e] = cur_flow[e] + ((double)flow[e]-cur_flow[e])/(steps-i);
    }
...
void WaitForAction(GraphWin& gw, float sec, string text) 
{ 
  if (sec == 0) text += "~~~\\blue (press done to continue)";
              ...
              edge_array <int> &flow, node_array <int> &excess) 
{
  ...
  if (do_anim) {
    gw.del_message();
    gw.message ("compute exact distance labels");
  }
  ...
  if (do_anim) {
    ...
    WaitForAction (gw,1.0,"dist labels computed");
    //WaitForAction (gw,0, "saturate all edges incident to s");
  }
  ...
  forall_adj_edges (e, s) {
	...
	//WaitForAction (gw,0, "push over next incident edge");
...
*/
	...
	showText(9); 
        showText(10); /*push*/ 
      	...
      	if (do_anim) {
      	  ...
      	  WaitForAction (gw,0, "push flow from red to orange node");
    ...
    if (excess[v] > 0) {
     ...
     dist[v] = min_dist + 1;
      ...
      if (do_anim) {
       ...
       WaitForAction (gw,0, "relabel red node");
  ...
  if (do_anim) {
    WaitForAction (gw, 2.0, "\\bf Finished.");
...
void start_maxflow(int) 
{
  ...
  gw.del_message();
  gw.message (string("\\bf MaxFlow = %d", mflow));
  gw.redraw ();
}
...
*/
  ...
  { gw.display(50,window::center);
    gw.set_animation_steps(32);
   }
  else
  { gw.display(window::center,window::center);
    gw.set_animation_steps(128);
   }

  if (!is_file(doc_file)) {
     gw.acknowledge("Cannot open " + doc_file);
  }
