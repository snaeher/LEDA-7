...
{ graph& G = gw.get_graph();
  ...
  forall_edges(e,G) 
    { if ( p[e] < min_cost) 
      { p[e] = rand_int(min_cost,max_cost); c[e] = p[e] - 1; }
  ...
  { mes = "Optimal cycle is shown in bold\\5 ";
    lambda_star = MINIMUM_RATIO_CYCLE(G,cost,profit,C_opt);
  }
...
{ p[e] = rand_int(min_cost,max_cost);
  c[e] = p[e] - 1;
...
{ 
  ...
  gw_add_simple_call(gw,about_minimum_ratio_cycle, "About Minimum Ratio Cycles",h_menu);

  about_minimum_ratio_cycle(gw);
  window& W = gw.get_window();
  min_cost = leda_max(2,W.read_int("minimum cost of an edge (at least two) = "));
  max_cost = leda_max(min_cost,W.read_int("maximum cost of an edge (at least minimum) = "));
