...
void show_all_graph_morphisms(GraphWin& gw1, MORPHISM morphism)
{
  ...
  if (k == 1) msg = string("There is \\bf one \\rm %s.", names[morphism]);
  if (k >= 2) msg = string("There are \\bf %d \\rm %ss.", k, names[morphism]);

  panel alert_box;
  alert_box.text_item(msg);
  alert_box.button("ok",1);
  if (k > 0)
    alert_box.button("show",2);
  ...
  if (alert_box.open(gw2.get_window()) == 2) 
  { 
    //show morphisms
...
int main()
{
  ...
  for(int a = 0; a < 2; a++) // only vf2 and conauto (s.n.)
     algorithms.push_back(ALGORITHM_NAMES[a]);
