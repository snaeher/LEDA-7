...
class node_pos : public node_position_base 
{
  const rat_point_set& ps;

public:

  node_pos(const rat_point_set& S) : ps(S) {}

  point operator()(node v) const 
  { return ps.pos(v).to_float(); }
};
...
//W.button("error",   3);
  ...
  //gw_observer observ(T,obs_w,obs_h);

  node_pos pos(T);
  gw_observer observ(T,pos,obs_w,obs_h);

  observ.win_init(0,256,0);
  observ.display(-W.xpos(),W.ypos());
  observ.set_edge_distance(3);
  observ.set_edge_width(1);
  observ.set_node_width(8);
  observ.set_node_height(8);
  observ.set_node_label_type(no_label);
