...
#include "src/TextLoader.h"
#include "src/TextLoader.c"
...
double delay=0.25;
TextLoader TL(500,460,"Preflow-Push Pseudo Code" );
...
void compute_dist (const GRAPH<int,int>& G, node s, node t,
...
void ComputeEdges (GRAPH<int,int>& G, node v, list <edge>& EL, const edge_array <int>& flow, const node_array <int>& dist, const node_array <int>& excess) {
  node w;
  edge e;
  int rcap;
              ...
              edge_array <int> &flow, node_array <int> &excess) 
{
  ...
  if (G.empty()) 
  { error_handler(0,"max_flow: empty graph.");
    return 0;
   }

  if (s == nil) 
  { error_handler(0,"max_flow: no source node defined.");
    return 0;
   }

  if (t == nil) 
  { error_handler(0,"max_flow: no sink node defined.");
    return 0;
   }
  ...
  if (do_anim) {
    gw.del_message();
    gw.message ("compute exact distances");
    //WaitForAction(gw,1.0,"compute excact distances");
  }
...
/*
  if (G.number_of_nodes () == 0) {
    ...
    s = G.first_node();
    t = G.succ_node(s);
    if (t == nil) error_handler(1,"need at least two nodes");
  }
*/
  ...
  t = 0;
  forall_nodes(v,G)
  { if (gw.get_label(v) == "s") s  = v;
    if (gw.get_label(v) == "t") t  = v;
   }
