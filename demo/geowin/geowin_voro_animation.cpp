...
public:
 ...
 my_animation() { counter=0; counter2=0; flag = true; pact = point(0,0); tx=5; ty=5; }
 
 virtual ~my_animation() { }

 bool is_running(const GeoWin& gw) { 
  return flag; 
 }
 ...
 point get_next_point(const GeoWin& gw) {
  ...
  if (counter%50 == 0) { tx=-tx; ty=-ty; }
  
  pact = pact.translate(tx,ty);  
  return pact;
 }
 
 long  get_next_action(const GeoWin& gw) { 
   counter2++;
   ...
   return GEOWIN_MOVE_SELECTED;
 }
 
 void finish(const GeoWin& gw) {
 }
};
...
int main()
{
  ...
  gw.add_special_help_text("geowin_voro", false);
  gw.display(window::center,window::center);
  
  my_animation anim; 
  
  gw.set_selected_objects(sc_input, rectangle(-400,-400,0,0));
  
  gw.animate(sc_input, anim);
