...
void geo_scene_algo(int n)
{
  ...
  if (cur) cur->call_buffer_fcn(n,true,cout);
}
...
void GeoWin::init_file_menu()
{
  int but,but1, but2, but3,but4, but5;
  ...
  but1 = add_call( new GeowinMember(&GeoWin::file_save_active_selected), "Save selected objects",menus[io_menu]);
  ...
  buttons[GEO_CM_WRITE_SEL]     = but1;
  button_info[GEO_CM_WRITE_SEL] = 1;
...
{ 
  ...
  add_call( new GeowinMember(&GeoWin::rotate_menu), "Rotate",  
	    menus[operate_menu]); 
  add_call( new GeowinMember(&GeoWin::transformation_menu), "Transform",  
	    menus[operate_menu]); 	    
  ...
  add_call(new GeoEditorMember(&GeoEditor::generate), "Generate", 
	   menus[edit_menu2]); 
  menus[edit_menu2]->new_line();	   
  add_call(new GeoEditorMember(&GeoEditor::transform_options), "Configure transformation", 
	   menus[edit_menu2]);	   
  add_call( new GeowinMember(&GeoWin::update_active), "Update active scene",
	    menus[edit_menu2]);   
   
  	    
}
  
void GeoWin::init_scenes_menu()
{
  // Scene menu  
  if( editables.empty() )
    {
  ...
  menus[scene_menu]->button("description", menu_const + scene_desc_menu,
...
void GeoWin::init_window_menu()
{
  ...
  add_call(new GeowinMember(&GeoWin::fill_window),"Fill &Window", 
	   menus[view_menu]);
  // new ----
  add_call(new GeowinMember(&GeoWin::zoom_undo), "Undo zoom",
           menus[view_menu]);	   
...
void GeoWin::init_options_menu()
{
  ...
  add_call(new GeowinMember(&GeoWin::advanced_global_options), "Advanced", 
	   menus[option_menu]);	   
  add_call(new GeowinMember(&GeoWin::d3_options), "3d Options", 
	   menus[option_menu]);
  ...
  add_call(new GeowinMember(&GeoWin::user_layer), "User layer", 
	   menus[option_menu]);  		 
...
void GeoWin::init_algorithms_menu()
{
...
// adding of special help texts
void GeoWin::add_special_help_text(string name, bool auto_display)
{
  help_list.append(name); 
  
  special_help_text = name;
  special_help_text_display = auto_display;
}
  
void GeoWin::init_help_menu()
{
...
void GeoWin::init_menu(window* wptr)
{
  if (menu_init) return;
  menu_init = true;
  ...
  if (wptr==NULL) WIN_PTR = Wp;
  else WIN_PTR = wptr;

  WIN_PTR->buttons_per_line(8);  
  int but;
  ...
  init_window_menu();
  init_options_menu();
  init_algorithms_menu();
  init_help_menu();
  
  
  // Main Menu
  if (show_menu) {
   if (show_file_menu)   WIN_PTR->button("File", menu_const + io_menu, *(menus[io_menu]));
   if (show_edit_menu)   WIN_PTR->button("Edit", menu_const + edit_menu1,*(menus[edit_menu1]));
   if (show_scenes_menu) WIN_PTR->button("Scenes", menu_const + scene_menu, *(menus[scene_menu]));
   if (show_window_menu) WIN_PTR->button("Window", menu_const + view_menu, *(menus[view_menu])); 
   if (user_fcn_count > 0) 
     WIN_PTR->button("User", menu_const + run_menu, *(menus[run_menu]));
  }
    
  // special help button ...
  if (special_help_text != "") {
    add_call( new GeowinMember(&GeoWin::special_help_button_pressed), "?", WIN_PTR );
  }     
    
  // done button ...  
  but = add_call( new GeowinMember(&GeoWin::done_button_pressed), "done", WIN_PTR ); 
  buttons[GEO_CM_OK]     = but;		  
	
  if (show_menu) {		  
   if (show_options_menu)    WIN_PTR->button("Options", menu_const + option_menu, *(menus[option_menu]));
   if (show_algorithms_menu) WIN_PTR->button("Algorithms", menu_const + algo_menu, *(menus[algo_menu]));
   if (show_help_menu)       WIN_PTR->button(" ? ", menu_const + help_menu, *(menus[help_menu]));
  }
  ...
  if (user_menu_add_handler) {
     user_menu_add_handler(*WIN_PTR);
  }

  //WIN_PTR->make_menu_bar(2);
}
...
void GeoWin::no_scene_on()
{
  ...
  menus[io_menu]->disable_button(buttons[GEO_CM_WRITE_SEL]);
...
void GeoWin::no_scene_off()
{
  ...
  menus[io_menu]->enable_button(buttons[GEO_CM_WRITE_SEL]);
...
gw.message("");
  ...
  gw.view_list.push_front(rectangle(W.xmin(),W.ymin(),W.xmax(),W.ymax()));
...
{
#if defined DEBUG
  cout << "geo_active_scene_input_options !\n";
#endif
...
void geo_zoom_up(GeoWin& gw, const point& p) { gw.zoom_up(); }
void geo_zoom_down(GeoWin& gw, const point& p) { gw.zoom_down(); }
...
void GeoWin::reset_actions()
{
  ...
  set_action( A_SCROLL_UP | A_IMMEDIATE, geo_zoom_up);
  set_action( A_SCROLL_DN | A_IMMEDIATE, geo_zoom_down);
