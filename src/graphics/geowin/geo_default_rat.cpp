...
class geowin_import_points : public geowin_import {
public:
  geowin_import_points(string n, string d) { name=n; description=d; } 

  void operator()(geo_scene sc, string filename) { 
     ...
     if (type_name != "Points") return;
     
     //geowin_load_leda_objects(f_objs, is);
     for(;;)
     { char c;
       while (is.get(c) && isspace(c)) {}
       if (!is) break;   
       is.putback(c);
       point obj;  is >> obj;  f_objs.push_back(obj);
     }       
     
     GeoWin* gw = get_geowin(sc);

     point piter;
     forall(piter, f_objs) r_objs.append(rat_point(piter));
     geowin_set_objects(*gw, sc, r_objs); 
  }
};

static geowin_import_points IRT("Import points", "Import object for float-kernel points");
