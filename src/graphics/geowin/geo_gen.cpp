...
void disc_points(list<point>& L, circle C, int N)
{
...
void geowin_generate_objects(GeoWin& gw, list<point>& L)      
{
  ...
  P.text_item("\\bf Point Generator");
  P.text_item("");
  ...
  //new ...
  int gen_count = gw.gen_count;
  int gen_count2= gw.gen_count2;
  if (gen_count<0 || gen_count>19) gen_count=0;
  if (gen_count2<0 || gen_count2>99) gen_count2=20;
  
  P.text_item("# points = 100 * a + b");
  P.text_item("");

  P.int_item("a = ", gen_count, 0, 19);
  P.int_item("b = ", gen_count2, 0, 99);
  ...
  int n = 100*gen_count+gen_count2;
    
  switch(but)
    {
    case 0 : break;
    case 1 :
     { random_points( L, n, x1, y1,  x2,  y2, grid);
       break;
     }
    case 2 :
     { w >> seg;
       segment_points(L,seg,n);
       break;
     }
    case 3 :
     { do { w >> cir; } while (cir.is_trivial());
       circle_points(Lr,cir,n);
       rat_point p;
       forall(p, Lr) L.append(p.to_point());
       break;
     }
    case 4 :
     { lattice_points(L,n,x1,y1,x2,y2);
       break;
     }
    case 5 :
     { w >> cir;
    ...
    case 6 :
     { w >> rect;
       rectangle_points(L,rect,n);
       break;
      }     
    case 7 :
     { w >> pol;
       polygon_points(L,pol,n);
       break;
     }
    case 8 :
     { // vertices of hilbert polygon
       int h = 0;
       while (n > 1) { h++; n /= 4; }
       hilbert(L,h,x1,y1,x2,y2,grid);
     } 
    default : //cout << but << endl; break;
     { // an additional generator was choosen ...
       gw.set_generator_number(c_scene,but-9);
       break;       
     }
  }
  
  gw.gen_count=gen_count; 
  gw.gen_count2=gen_count2;
}
...
{ 
  ...
  P.text_item("\\bf Segment Generator");
  P.text_item("");
  ...
  switch(but)
    ...
    case 0 : break;
    case 1 :
     { random_points( L, 2*n, x1, y1,  x2,  y2, grid);
      lit=L.first();
      while(lit)
	{
    ...
    case 3 :
     { 
      do { w >> cir; }
      while (cir.is_trivial());
      
      circle_points(rL, cir, n);
      lit=rL.first();
...
{   
  ...
  P.text_item("\\bf Circle Generator");
  P.text_item("");
...
{  
  ...
  P.text_item("\\bf Polygon Generator");
  P.text_item("");

  int n = gw.gen_count;
  if (n < 3) n = 3;
  
  P.int_item("size", n, 3, 1000);
...
void geowin_generate_objects(GeoWin& gw, list<gen_polygon>& Pl) 
{
  ...
  P.text_item("\\bf Generalized Polygon Generator");
  P.text_item("");
...
void geowin_generate_objects(GeoWin& gw, list<d3_point>& L)
{
  ...
  P.text_item("\\bf D3-Point Generator");
  P.text_item("");
  ...
  switch (but) {
   case 0: return;
   case 1: {
     do { w >> circ; } while (circ.is_trivial());
     cpoint = circ.center();
     random_points_in_cube(n,(int)(circ.radius()),Lr); 
     break;
   }
   case 2: {
     do { w >> circ; } while (circ.is_trivial());
     cpoint = circ.center();
     random_points_in_ball(n,(int)(circ.radius()),Lr); 
     break;
   }
   case 3: {
     do { w >> circ; } while (circ.is_trivial());
     cpoint = circ.center();
     random_points_in_square(n,(int)(circ.radius()),Lr); 
     break;
   }
   case 4: {
     do { w >> circ; } while (circ.is_trivial()); 
     cpoint = circ.center();
     random_points_on_paraboloid(n,(int)(circ.radius()),Lr); 
     break;
   }
   case 5: { 
     do { w >> circ; } while (circ.is_trivial()); 
     cpoint = circ.center();
     lattice_points(n,(int)(circ.radius()),Lr); 
     break; 
   }
   case 6: {
     do { w >> circ; } while (circ.is_trivial()); 
     cpoint = circ.center();
     random_points_on_sphere(n,(int)(circ.radius()),Lr); 
     break;
   }
   case 7: {
     do { w >> circ; } while (circ.is_trivial());
     cpoint = circ.center();
     random_points_on_segment(n,(int)(circ.radius()),Lr); 
     break;
   }
   default : 
     { // an additional generator was choosen ...
