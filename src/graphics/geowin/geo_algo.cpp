...
void edit_gen_polygon(GeoWin& gw, gen_polygon& gp, int i)
{
  ...
  do {
   ...
   list_item iter1, iter2, found1=NULL;
   int counter = 0;
   int found2 = 0;
   ...
   if (but == MOUSE_BUTTON(1)){ // left ...
      // delete vertex ...
      forall_items(iter1,LP){
        const list<point>& pts = LP[iter1].vertices();
	
	counter=0;
	forall_items(iter2, pts){
          double xw = pts[iter2].xcoord(), yw = pts[iter2].ycoord();
          if ((xw>=x1) && (xw<=x2) && (yw>=y1) && (yw<=y2)) {  found1=iter1; found2=counter; }
	  counter++;
	}
      }
      if (found1 != NULL) {  // delete a vertex ...
         polygon& pol = LP[found1];
	 
	 list<point> LPts = pol.vertices();
	 list_item it = LPts.get_item(found2);
	 
	 LPts.del_item(it);
         pol = polygon(LPts, polygon::NO_CHECK);
	 gp = gen_polygon(LP, gen_polygon::NO_CHECK);
	 gw.get_active_scene()->update_and_redraw();
      }   
      else {
       // try to insert a vertex ...
       forall_items(iter1,LP) {
        LS = LP[iter1].segments();

	counter=0;
	forall_items(iter2, LS){       
          if (geowin_IntersectsBox(LS[iter2],x1,y1,x2,y2,true)) { found1=iter1; number=counter; }
	  counter++;
	}
       }
       if (found1 != NULL) { // insert a vertex ...
         polygon& pol = LP[found1];
       
         list<point> LPts = pol.vertices();
         list_item it = LPts.get_item(number);
	 LPts.insert(point(x,y),it);
	 pol = polygon(LPts, polygon::NO_CHECK);
	 gp = gen_polygon(LP, gen_polygon::NO_CHECK);
	 gw.get_active_scene()->update_and_redraw();         
       }
...
void edit_polygon(GeoWin& gw, polygon& gp, int i)
{
  ...
  do {
   ...
   if (but == MOUSE_BUTTON(1)){ // left ...
      ...
      if (found != NULL) { 
         LP.del_item(found);
	 gp = polygon(LP, polygon::NO_CHECK);
...
{
  list<rat_polygon> LP = gp.polygons();
  list<rat_segment> LS;
  ...
  do {
   ...
   if (but == MOUSE_BUTTON(1)){ // left ...
      // delete vertex ...
      forall_items(iter1,LP){
        const list<rat_point>& pts = LP[iter1].vertices();
	
	counter=0;
	forall_items(iter2, pts){
          double xw = pts[iter2].to_float().xcoord(), yw = pts[iter2].to_float().ycoord();
          if ((xw>=x1) && (xw<=x2) && (yw>=y1) && (yw<=y2)) {  found1=iter1; found2=counter; }
	  counter++;
	}
      }
      if (found1 != NULL) {  // delete a vertex ...
         rat_polygon& pol = LP[found1];
	 
	 list<rat_point> LPts = pol.vertices();
	 list_item it = LPts.get_item(found2);
	 
	 LPts.del_item(it);
         pol = rat_polygon(LPts, rat_polygon::NO_CHECK);
	 gp = rat_gen_polygon(LP, rat_gen_polygon::NO_CHECK);
	 gw.get_active_scene()->update_and_redraw();
      }   
      else {
       // try to insert a vertex ...
  ...
  do {
   ...
   if (but == MOUSE_BUTTON(1)){ // left ...
      ...
      if (found != NULL) { // delete a vertex ...
         LP.del_item(found);
	 gp = rat_polygon(LP, rat_polygon::NO_CHECK);
	 LP = gp.vertices(); LS = gp.edges();
	 gw.get_active_scene()->update_and_redraw();
      }
      else {
