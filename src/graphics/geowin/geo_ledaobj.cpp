...
{ 
  ...
  obj = polygon(PL, polygon::NO_CHECK);
}
...
{ 
  list<polygon> Polys = obj.polygons();

  list_item it;
  forall_items(it, Polys){
   polygon& pact = Polys[it];
   if (pnr >= pact.size()) pnr=pnr-pact.size();
   else { // we found the right polygon !!
   
     geowin_Translatepoint_poly(pact, dx, dy, pnr);
     Polys[it] = pact;
     obj = gen_polygon(Polys, gen_polygon::NO_CHECK);
     return;
   }
  }
}
...
{ 
  ...
  if (x1 > x2) leda_swap(x1,x2);
  if (y1 > y2) leda_swap(y1,y2);
...
bool geowin_IntersectsBox(const geo_paint_object& obj, double x1, double y1,
		          double x2, double y2)
{ 
  const list<point>&     L1 = obj.get_container();  
  const list<point>&     L2 = obj.get_user_points();
  const list<segment>&   L3 = obj.get_user_segments();
  const list<rectangle>& L4 = obj.get_user_rectangles();
  const list<circle>&    L5 = obj.get_user_circles();  

  point pi;
  forall(pi, L1) if (geowin_IntersectsBox(pi, x1, y1, x2, y2, true)) return true;
  forall(pi, L2) if (geowin_IntersectsBox(pi, x1, y1, x2, y2, true)) return true;

  segment si;
  forall(si, L3) if (geowin_IntersectsBox(si, x1, y1, x2, y2, true)) return true;

  rectangle ri;
  forall(ri, L4) if (geowin_IntersectsBox(ri, x1, y1, x2, y2, true)) return true;

  circle ci;
  forall(ci, L5) if (geowin_IntersectsBox(ci, x1, y1, x2, y2, true)) return true;
