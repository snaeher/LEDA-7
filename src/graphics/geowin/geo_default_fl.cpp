...
class geowin_import_rat_points : public geowin_import {
public:
  geowin_import_rat_points(string n, string d) { name=n; description=d; } 
  ...
  void operator()(geo_scene sc, string filename) { 
     ...
     if (type_name != "RationalPoints") return;
     
     //geowin_load_leda_objects(r_objs, is);
     for(;;)
     { char c;
       while (is.get(c) && isspace(c)) {}    
       if (!is) break;   
       is.putback(c);
       rat_point obj;  is >> obj;  r_objs.push_back(obj);
     }  
...
static geowin_import_rat_points IRP("Import rat_points", "Import object for rational points");
