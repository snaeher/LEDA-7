...
{ 
  ...
  node v = source(answer);
  node w = target(answer);

  if ( contains(pos(v),pos(w),p) && ( is_hull_dart(answer)
       || (!is_hull_dart(answer) && !is_hull_dart(reversal(answer)) ))) 
  return;

  if (orientation(answer,p) < 0) 
  { cerr << "\norientation(" << pos(v) << "," << pos(w) << "," << p << ") < 0.";
...
{ 
  ...
  { 
    ...
    int d = orientation(pos_source(e),p,pos_target(e1));
...
{ if ( check ) save_state(p);
  ...
  bool p_on_e = contains(pos(source(e)),pos(target(e)),p);
...
node POINT_SET::nearest_neighbor(node v) const
{
  ...
  { node u = target(e);  
    if ( cmp_dist(p,pos(u),pos(min_v)) < 0 ) min_v = u;
  }
...
node POINT_SET::nearest_neighborA(node v) const
{
  ...
  node min_v = target(e);
  RAT_TYPE min_d = sqr_dist(p,pos(min_v));

  while ((e = adj_succ(e)) != nil)
  { node u = target(e); 
    RAT_TYPE d_u = sqr_dist(p,pos(u)); 
    if ( d_u < min_d ) 
    { min_v = u;
      min_d = d_u;
    }
...
{ 
  ...
  { node v = L.pop();
    ...
    if ( sqr_dist(p,pos(v)) < min_d )
    { min_v = v;
      min_d = sqr_dist(p,pos(v));
    }
...
{ L.append(s);
  ...
  forall_adj_nodes(u,s)
      if (!is_marked(u) && cmp_dist(pv,pos(u),p) <= 0 ) dfs(u,pv,p,L);
}
...
void POINT_SET::dfs(node s, const POINT& a, const POINT& b, const POINT& c,
                                                            list<node>& L) const
{ L.append(s);
  mark_node(s);
  node u;
  forall_adj_nodes(u,s)
      if (!is_marked(u) && !outcircle(a,b,c,pos(u))) dfs(u,a,b,c,L);
}
 
 
list<node> POINT_SET::circular_range_search(const POINT& a,
                                            const POINT& b,
                                            const POINT& c)
{ 
  list<node> L;
  ...
  if ( number_of_nodes() == 1 && side_of_circle(a,b,c,pos(first_node())) * orient >= 0 ) 
  { L.append(first_node());
    return L;
  }
...
list<node> POINT_SET::triang_range_search(const POINT& a, 
                                          const POINT& b,
                                          const POINT& c) 
{ int orient = orientation(a,b,c);
  list<node> L = circular_range_search(a,b,c);
...
int cmp_edge_length(const edge& e1, const edge& e2)
{ RAT_TYPE l1 = T_tmp->sqr_dist(T_tmp->pos_source(e1),T_tmp->pos_target(e1)); 
  RAT_TYPE l2 = T_tmp->sqr_dist(T_tmp->pos_source(e2),T_tmp->pos_target(e2)); 
