...
atomic_counter triangle_rep::id_counter(0);

triangle_rep::triangle_rep(const point& p1,const point& p2,const point& p3) : a(p1),b(p2),c(p3)
{
 id=++id_counter;
}


triangle::triangle(const point&  Ap,const point& Bp,const point& Cp)
{
  PTR = new triangle_rep(Ap,Bp,Cp);
}

triangle::triangle(double x1,double y1,double x2,double y2,double x3,double y3) 
{
 point a(x1,y1),b(x2,y2),c(x3,y3);  
 PTR = new triangle_rep(a,b,c);
}
...
int triangle::side_of(const point& p) const
{
...
region_kind triangle::region_of(const point& p) const
{
  ...
  if (ori1*ori2*ori3 == 0){ // ON_REGION or UNBOUNDED_REGION
    if ((ori1==0) && (ori2==0) && (ori3==0)){ // triangle is degenerate
      if (segment(point1(),point2()).contains(p) || segment(point2(),point3()).contains(p) ||  \
          segment(point3(),point1()).contains(p)) return ON_REGION;
...
bool   triangle::outside(const point& p) const
{ return side_of(p) == -1; }

bool   triangle::on_boundary(const point& p) const
{ return side_of(p) == 0; }
...
bool   triangle::intersection(const line& l) const
{
...
bool   triangle::intersection(const segment& s) const
{
 segment s1(point1(), point2());
 segment s2(point2(), point3());
 segment s3(point3(), point1());
...
triangle triangle::rotate(const point& q, double a) const
{
 point p1=point1();
 point p2=point2();
 point p3=point3();
 return triangle(p1.rotate(q,a), p2.rotate(q,a), p3.rotate(q,a));
}

triangle triangle::rotate(double a) const
{
 point p1=point1();
 point p2=point2();
 point p3=point3();
 return triangle(p1.rotate(a), p2.rotate(a), p3.rotate(a));
}

triangle triangle::rotate90(const point& q, int i) const
{
 point p1=point1();
 point p2=point2();
 point p3=point3();
 return triangle(p1.rotate90(q,i), p2.rotate90(q,i), p3.rotate90(q,i));
}

triangle triangle::rotate90(int i) const
{ return rotate90(point(0,0),i); }


triangle triangle::reflect(const point& p, const point& q) const
{
 return triangle(point1().reflect(p,q), point2().reflect(p,q),point3().reflect(p,q));
}

triangle triangle::reflect(const point& p) const
{
 point p1=point1();
 point p2=point2();
 point p3=point3();
 return triangle(p1.reflect(p),p2.reflect(p),p3.reflect(p)); 
}
...
triangle triangle::translate(double wx,double wy) const
{
   point a=point1();
   point b=point2();
   point c=point3();
