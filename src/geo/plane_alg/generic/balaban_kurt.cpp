...
static void TreeSearch_nloghoch2n(slist<SEGMENT>& L, slist<SEGMENT>& I, 
      int b, int e, slist<SEGMENT>& R,void (*report)(const SEGMENT&,
                                     const SEGMENT&));

static void SearchInStrip(slist<SEGMENT>& L, slist<SEGMENT>& R, int b, int e,void (*report)(const SEGMENT&,
                                     const SEGMENT&));

static void InnerIntPoints(const STAIRCASE& D, const slist<SEGMENT>& I,
void (*report)(const SEGMENT&,
                                     const SEGMENT&));
...
static void Split(slist<SEGMENT>& L, STAIRCASE& D,void (*report)(const SEGMENT&,
                                     const SEGMENT&));
static void Lrs_from_Rls(slist<SEGMENT>& Rls, slist<SEGMENT>& Lrs, int c,void (*report)(const SEGMENT&,
                                     const SEGMENT&));
static void Merge(STAIRCASE& D, slist<SEGMENT>& R,void (*report)(const SEGMENT&,
                                     const SEGMENT&));
...
//static void (*report_func)(const SEGMENT&,const SEGMENT&) = 0;
...
static void balaend(const list<SEGMENT>& S, int N)
{ // free used memory
  
#if defined(__GNUC__) && (__GNUC_MINOR__ == 8)
  if (init_points)
  { for(int i=0; i<=N; i++) init_points[i].~POINT();
    free(init_points); 
   }
  if (stairs)
  { for(int i=0; i<=S.length(); i++) stairs[i].~SEGMENT();
    free(stairs);      
   }
#else
...
#if defined(__GNUC__) && (__GNUC_MINOR__ == 8)
//work-around for a bug in gcc-2.8
  init_points = (POINT*)malloc((N+1)*sizeof(POINT));
  stairs = (SEGMENT*)malloc((S.length()+1)*sizeof(SEGMENT));
  if ( !init_points || !stairs )  
    LEDA_EXCEPTION(1,"balaban: out of memory.");
  int i;
  for(i=0; i<=N; i++) new(init_points+i) POINT;
  for(i=0; i<=S.length(); i++) new(stairs+i) SEGMENT;
#else
  ...
  // start $TreeSearch$
 if( N > 1 )
 { TreeSearch_nloghoch2n(L, I, 1, N, R,report);
   Lrs_from_Rls(R, L, N, report);
  }
...
void TreeSearch_nloghoch2n(slist<SEGMENT>& Lv, slist<SEGMENT>& Iv,
			   int b, int e, slist<SEGMENT>& Rv,void (*report)(const SEGMENT&,
                                     const SEGMENT&))
{   
  // 1. if e-b = 1 then SearchInStripbe(Lv, Rv); exit;
  
  if( e-b == 1 )  { SearchInStrip(Lv, Rv, b, e, report); return; }
  ...
  TreeSearch_nloghoch2n( Lv, Ils, b, c, Rv,report);

  // 7. If pc is left endpoint of s(c) Lrs := insert s(c) into Rls
  //                           else    Lrs := remove s(c) from Rls
  
  Lrs_from_Rls(Rv, Lv, c, report); 

  // 8. TreeSearch(Lrs, Irs, c, e, Rrs);

  TreeSearch_nloghoch2n( Lv, Irs, c, e, Rv,report);
                                     ...
                                     const SEGMENT&))
{
  ...
  if( !L.empty() ) 
    ...
    else SearchInStrip(L, R, b, e, report);
...
void InnerIntPoints(const STAIRCASE& D, const slist<SEGMENT>& I,void (*report)(const SEGMENT&,
                                     const SEGMENT&))
...
void ReportIntersection(const SEGMENT& s1, const SEGMENT& s2,                      void (*report)(const SEGMENT&, const SEGMENT&))
{
  if ( report ) report( (*original)[s1],(*original)[s2] );
}

void Split(slist<SEGMENT>& L, STAIRCASE& D,void (*report)(const SEGMENT&,
                                     const SEGMENT&))
{ 
  ...
  forall(s, L)
    {
      loc = locs.inf(lit); lit = locs.succ(lit);
      if( loc < 0 )
	{
	  i = -loc;
	  // intersections between s and stairs below s
	  ReportIntersection(D[i--], s,report); // s would be a stair if not	  
...
void Lrs_from_Rls(slist<SEGMENT>& R, slist<SEGMENT>& L, int c,void (*report)(const SEGMENT&,
                                     const SEGMENT&))
{    
  SEGMENT         s; 
  POINT           pc(init_points[c]);
  ...
  while ( lit && !orientation(R.inf(lit), pc) ) 
	...
	{
	  forall(si, endings)    ReportIntersection(s, si,report);
	  forall(si, pass_start) ReportIntersection(s, si,report);
	  endings.append(s);
	  lit = L.succ(lit);
	...
	{  
	  ...
	  forall(si, pass_start)
	    if( orientation(s, si.start()) ) ReportIntersection(s, si,report);
	  pass_start.push(s); 
	  lit = L.succ(lit);
	  if ( lits_prev )  L.del_succ_item(lits_prev);
	  else L.pop(); 
  ...
  while(!seg_queue.empty() && identical(seg_queue.head().start(), pc))
      ...
      if ( identical( s.end(), pc ) ) // zero length segments
	{
	  forall(si, endings)    ReportIntersection(s, si,report);
	  forall(si, pass_start) ReportIntersection(s, si,report);
	  forall(si, nuller)     ReportIntersection(s, si,report);
	  nuller.append(s);
	}
	  ...
	  while(lit1)
	    {
	      ...
	      if(o) ReportIntersection(s, si,report);
	      if( o > 0) lit2 = lit1; 
...
void Merge(STAIRCASE& D, slist<SEGMENT>& R,void (*report)(const SEGMENT&,
                                     const SEGMENT&))
