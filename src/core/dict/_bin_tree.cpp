...
bin_tree_node* bin_tree::locate(GenPtr x) const { return  search(x); }
...
{ bin_tree_node* p = search(x);
  return (p && cmp(x,p->k) < 0) ? pred(p) : p ;
 }



//------------------------------------------------------------------------------
// search(x) : returns pointer to leaf with minimal key >= x (nil if T empty)
//------------------------------------------------------------------------------
...
{ 
  ...
  switch (type_id) 
  {
   ...
   case FLOAT_TYPE_ID: {
      float dx =  LEDA_ACCESS(float,x);
      while (p->is_node())
         p = p->child[ (dx <= LEDA_ACCESS(float,p->k)) ? left : right ];
      break;
    }
